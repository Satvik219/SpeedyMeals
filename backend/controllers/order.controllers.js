...
export const placeOrder = async (req, res) => {
    try {
        ...
        cartItems.forEach(item => {
            // Search results populate `shop`, while the regular item list returns
            // its id. Accept both shapes before using the value as an object key.
            const shopId = item.shop?._id || item.shop
            if (!shopId) {
                throw new Error(`shop is missing for cart item ${item.name || item.id}`)
            }
